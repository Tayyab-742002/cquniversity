...
export default function TestsPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto py-6 px-2 sm:px-4 md:py-8">
      {/* Welcome Header */}
      <div className="text-center mb-8">
        <div className="flex flex-col sm:flex-row items-center justify-center mb-4 gap-3 sm:gap-6">
          {user?.imageUrl && (
            <img 
              src={user.imageUrl} 
              alt="Profile" 
              className="w-16 h-16 rounded-full border-2 border-primary/20 mb-2 sm:mb-0 mr-0 sm:mr-3"
            />
          )}
          <div className="text-center sm:text-left">
            <h1 className="text-2xl sm:text-3xl font-bold">Welcome, {user?.firstName || 'Participant'}!</h1>
      ...
      <div className="text-center mb-10">
        <h2 className="text-xl sm:text-2xl font-bold mb-3">Psychological Tests</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {tests.map((test) => {
          ...
          return (
            <div 
              key={test.id}
              className={`bg-card border border-border rounded-lg shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md flex flex-col h-full ${
              ...
              <div className="p-4 sm:p-6 flex flex-col h-full">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-2 sm:gap-0">
                  <div className="flex items-center">
                    ...
                    <div>
                      <h3 className="text-lg sm:text-xl font-semibold">{test.name}</h3>
                      <p className="text-xs sm:text-sm text-muted-foreground">{test.duration} • {test.difficulty}</p>
                    </div>
                  </div>
                  {isCompleted && (
                  ...
                  {!test.implemented && (
                    <div className="text-xs sm:text-sm text-muted-foreground px-2 py-1 bg-muted rounded-md mt-2 sm:mt-0">
                      Coming soon
                    </div>
                  )}
                </div>
                ...
                <div className="mt-auto">
                  {test.implemented ? (
                    <Link 
                      href={test.path}
                      className={`inline-flex items-center px-4 py-2 rounded-md transition-colors w-full sm:w-auto text-center justify-center ${
                        isCompleted 
                    ...
                    <button 
                      disabled
                      className="inline-flex items-center px-4 py-2 bg-muted text-muted-foreground rounded-md cursor-not-allowed w-full sm:w-auto justify-center"
                    >
                      Not Available Yet
                    </button>
                  )}
