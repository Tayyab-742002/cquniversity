...
function TrailMakingTestContent() {
  ...
  useEffect(() => {
    ...
    const checkPreviousResults = async () => {
      try {
        console.log('🔍 Checking for previous Trail Making test results...');
        const response = await axios.get(`/api/test-results/check?participantId=${participantId}&testId=trailMakingTest`);
...
export default function TrailMakingTestPage() {
  return (
    <DeviceAccessGuard testName="Trail Making Test">
      <TrailMakingTestContent />
    </DeviceAccessGuard>
  );
} 