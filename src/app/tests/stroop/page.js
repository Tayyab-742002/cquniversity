...
function StroopTestContent() {
  ...
  useEffect(() => {
    ...
    const checkPreviousResults = async () => {
      try {
        console.log('🔍 Checking for previous Stroop test results...');
        const response = await axios.get(`/api/test-results/check?participantId=${participantId}&testId=stroopTest`);
...
export default function StroopTestPage() {
  return (
    <DeviceAccessGuard testName="Stroop Test">
      <StroopTestContent />
    </DeviceAccessGuard>
  );
} 